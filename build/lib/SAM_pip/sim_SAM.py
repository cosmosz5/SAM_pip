...
def simSAM_PSF (data_filename, mask_filename, wave, bandwidth, hole_size, px_scale, imsize, hole_geom, source, inst, arrname, rotation_angle, oversample):
    import numpy as np
    import astropy.io.fits as pyfits
    from . import sam_tools
    from . import reduce_SAM_poly2
    from .readcol import readcol as readcol
    from itertools import combinations

    [data_files] = readcol(data_filename, twod=False)
    [x_coord, y_coord] = readcol(mask_filename, twod=False, skipline=2)
    ###### AUTOMATIC FROM HERE ####
    ###############################
    px_scale = float(px_scale)
    im = np.zeros([imsize, imsize])
    nholes = x_coord.shape[0]
    ...
    if inst == 'NACO':
        # x_coord = x_coord * 8. / 10 ### To convert from millimeters to meters
        # y_coord = y_coord * 8. / 10
        x_coord = -1 * x_coord
        # hole_size = hole_size * 8. /10
    if inst == 'VISIR':
        x_coord = x_coord * 0.61 / 1.4  ### To convert from millimeters to meters
        y_coord = y_coord * 0.61 / 1.4
        y_coord = -1 * y_coord
        x_coord = -1 * x_coord
    if inst == 'SPHERE':
        x_coord = 1.0 * x_coord
    ...
    if (inst == 'VISIR') | (inst == 'JWST'):
        ...
        dd = 1.22 * wave / (hole_size * sam_tools.mas2rad(px_scale))
    window = np.fft.fftshift(np.exp(- (sam_tools.dist(imsize) / dd) ** 4))
    ind_window = np.where(window > 0.08)
    uwindow = np.zeros([imsize, imsize])
    uwindow[ind_window] = 1.0
    # ind_window2 = np.where(window > 0.02)
    # uwindow2  = np.zeros([imsize, imsize])
    # uwindow2[ind_window2] = 1.0
    #
    # ring_window = uwindow2 - uwindow
    # median_noise = np.median(ring_window)
    # std_noise = np.std(ring_window)
    #
    # ind_window = np.where((PSF_im * uwindow) > median_noise)
    # uwindow  = np.zeros([imsize, imsize])
    # uwindow[ind_window] = 1.0
    ...
    for i in range(len(data_files)):
        print(data_files[i])
        reduce_SAM_poly2.redSAM(data_files[i], PSF_im, uwindow, bl_x, bl_y, wave, hole_size, imsize, px_scale, nholes,
                                nbl, ncp, \
                                baselines, closure_phases, x, y, air, cube_bl, cube_bl_sin, x_coord, y_coord, source,
